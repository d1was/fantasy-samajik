      ...
      <header class="bg-white shadow">
        <div class="max-w-screen-xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
          ...
          >
            🏆 Manager of the Week:
            <i>
              {{ week.champion.mgr_name }} ({{
                week.champion.live_points
              }}
              points)
            </i>
          </h3>
          ...
          >
            🐔 Flop of the Week:
            <i>{{ week.flop.mgr_name }} ({{ week.flop.live_points }} Point)</i>
          </h3>
          <h3
            class="text-2xl font-bold leading-tight text-gray-900"
            v-if="week.flop"
          >
            🧠 Mastermind of the Week:
            <i
              >{{ mastermind[0].name }} (Transfer points:
              {{ mastermind[0].net_transfer_points }} Point, Transfers:
              {{ mastermind[0].transfers }})</i
            >
          </h3>
          <h3
            class="text-2xl font-bold leading-tight text-gray-900"
            v-if="week.flop"
          >
            🤖 Overthinker of the Week:
            <i
              >{{ mastermind[mastermind.length - 1].name }} (Transfer points:
              {{ mastermind[mastermind.length - 1].net_transfer_points }} Point,
              Transfers: {{ mastermind[mastermind.length - 1].transfers }})</i
            >
          </h3>
        </div>
      </header>
      ...
      <main>
        <div class="px-4 py-6 sm:px-0 flex flex-col">
          ...
          >
            ...
            <table
              class="w-full overscroll-auto mt-4 border-2 border-green-800 border-collapse table-auto text-blue-800"
              border=""
            >
              <thead>
              ...
              <tbody>
                <tr
                  class=""
                  v-for="manager in sortedManagers"
                  :key="manager.entry"
                >
                  ...
                  <td class="border-2 border-gray-200 p-1">
                    {{ manager.live_overall_points }}
                  </td>
                  <td class="border-2 border-gray-200 p-1">
                    {{ manager.gw_points - manager.gw_transfers_cost }}
                  </td>
                  <td class="border-2 border-gray-200 p-1">
                    {{ topPoint - manager.live_overall_points }}
                  </td>
                  <td class="border-2 border-gray-200 p-1">
                    {{ manager.current_overall_rank }}
                  </td>
                </tr>
              </tbody>
            </table>
          ...
          >
            ...
            <table
              class="w-full mt-4 border-2 border-green-800 border-collapse table-auto text-blue-800"
              border=""
            >
              <thead>
                <tr class="border-2 border-gray-200 p-1">
              ...
              <tbody>
                <tr class="" v-for="manager in managers" :key="manager.entry">
                  ...
                  <td class="border-2 border-gray-200 p-1">
                    {{ manager.captain.display_name }}
                  </td>
                  <td class="border-2 border-gray-200 p-1">
                    {{ getCaptainScore(manager.captain) }} Point
                  </td>
                </tr>
              </tbody>
            </table>
...
export default defineComponent({
  ...
  mounted() {
    ...
    this.$data.managers = apiData.managers.map((manager) => ({
      ...
      live_rank: manager.live_rank,
      live_overall_points: manager.live_overall_points,
    }));
    console.log(
      [...this.$data.managers.sort((a, b) => a.live_rank - b.live_rank)],
      "asds"
    );

    this.$data.gw = apiData.managers[0].gw;
    let sortedManager = apiData.managers.sort(
      (a, b) => b.live_points - a.live_points
    );
    this.$data.week = {
  ...
  computed: {
    sortedManagers() {
      let managers = [
        ...apiData.managers.sort((a, b) => a.current_rank - b.current_rank),
      ];
      return managers;
    },
    topPoint() {
      return apiData.managers.sort(
        (a, b) => b.live_overall_points - a.live_overall_points
      )[0].live_overall_points;
    },
    mastermind() {
      return this.managers.sort(
        (a, b) => b.net_transfer_points - a.net_transfer_points
      );
    },
  },
  data: () => ({
  ...
  methods: {
    getCaptain(manager) {
      ...
      let player = this.players.find(
        (player) => player.id == captainId.element
      );
      return player;
    },
